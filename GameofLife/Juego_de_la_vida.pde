...
void keyPressed() {     // ESTE PROCESO SE VINCULA AL AUMENTO Y DISMINUCION DE LA VELOCIDAD DEL PROGRAMA AL IGUAL QUE SU PAUSA Y SU REINCIO.SE IMPLEMENTAN DICHAS 
  if (key == CODED) {   // OPERACIONES CON EL TECLADO.EL AUMENTO O LA DISMINUCION DE LA VELOCIDAD SE HACE CON LAS TECLAS "ARRIBA" Y "ABAJO"
    if (keyCode == UP) {
      frame = frame+1;
  ...
  if (k == 'S')
    if (looping)  noLoop();   // Esta funcion pausa el ciclo del void draw.Con la tecla s se pausa y se reanuda la ejecucion del programa.
    else          loop();
}